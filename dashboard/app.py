...
===================================
Gọi report_api (Flask) tại http://report_api:5000/api/report
Response format:
{
    "success": true,
    "total_records": N,
...
# ── Config ────────────────────────────────────────────────────
# Trong Docker: report_api là tên service trong docker-compose
# Khi dev local: set biến môi trường REPORT_API_URL=http://localhost:5001
REPORT_API_URL = os.getenv("REPORT_API_URL", "http://report_api:5000")
USE_MOCK        = os.getenv("USE_MOCK", "false").lower() == "true"


# ── Mock data để dev UI khi chưa có backend ──────────────────
MOCK_DATA = {
    "success": True,
    "total_records": 6,
    "data": [
        {"order_id": 1, "product_name": "iPhone 15 Pro",     "status": "Success", "quantity": 2, "unit_price": 25990000, "total_revenue": 51980000},
        {"order_id": 2, "product_name": "Samsung Galaxy S24", "status": "Success", "quantity": 1, "unit_price": 21990000, "total_revenue": 21990000},
        {"order_id": 3, "product_name": "MacBook Air M3",     "status": "Success", "quantity": 1, "unit_price": 32990000, "total_revenue": 32990000},
        {"order_id": 4, "product_name": "Sony WH-1000XM5",   "status": "Success", "quantity": 3, "unit_price":  8490000, "total_revenue": 25470000},
        {"order_id": 5, "product_name": "iPad Pro M2",        "status": "Success", "quantity": 2, "unit_price": 24990000, "total_revenue": 49980000},
        {"order_id": 6, "product_name": "Dell XPS 15",        "status": "Success", "quantity": 1, "unit_price": 28990000, "total_revenue": 28990000},
    ]
}
...
def fetch_report():
    ...
    try:
        headers = {"apikey": "noah-secret-key-2026"}
        resp = requests.get(f"{REPORT_API_URL}/api/report", headers=headers, timeout=10)
        resp.raise_for_status()
        return resp.json(), None
    except requests.exceptions.ConnectionError:
        return None, "Không kết nối được report_api. Kiểm tra Docker đang chạy chưa."
    except requests.exceptions.Timeout:
        return None, "Report API timeout (>10s)."
...
st.caption("Module 6 | CMU-CS 445 System Integration Practices")
...
if error:
    ...
    st.info("💡 Tip: Chạy `docker compose up` để khởi động backend, hoặc set `USE_MOCK=true`.")
    st.stop()
...
display_df = display_df.rename(columns={
    "order_id": "Mã đơn", "product_name": "Sản phẩm",
    "status": "Trạng thái", "quantity": "Số lượng",
    "unit_price": "Đơn giá", "total_revenue": "Doanh thu",
})
...
with cc1:
    ...
    top3 = df.nlargest(3, "total_revenue")[["order_id", "product_name", "total_revenue"]]
